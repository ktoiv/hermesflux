...
const EXPENSE_MAP: Record<string, string> = {
  housing: 'Housing', food: 'Food & Drink', transport: 'Transport',
  shopping: 'Shopping', bills: 'Bills & Utilities', health: 'Health',
...
const INCOME_MAP: Record<string, string> = {
  ...
  gift: 'Gift', refund: 'Refund', bonus: 'Bonus',
...
export async function getCategoryIdByFormId(formId: string, type: 'expense' | 'income'): Promise<number | null> {
  ...
  const row = await db.getFirstAsync<{ id: number }>(
    'SELECT id FROM categories WHERE name = ? AND type = ?', [name, type],
  );
  return row?.id ?? null;
}
...
export async function createCategory(data: CreateCategory): Promise<Category> {
  ...
  const result = await db.runAsync(
    ...
    [data.name, data.type, data.icon ?? null, data.color ?? null],
  );
