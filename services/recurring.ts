...
export async function createRecurring(data: CreateRecurring): Promise<Recurring> {
  ...
  const result = await db.runAsync(
    `INSERT INTO recurring (name, amount, type, category_id, frequency, start_date, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [parsed.name, parsed.amount, parsed.type, parsed.category_id ?? null,
     parsed.frequency, parsed.start_date, parsed.notes ?? null],
  );
...
export async function updateRecurring(id: number, data: UpdateRecurring): Promise<Recurring | null> {
  ...
  await db.runAsync(
    ...
    [merged.name, merged.amount, merged.type, merged.category_id ?? null,
     merged.frequency, merged.start_date, merged.active, merged.notes ?? null, id],
  );
...
export async function generateTransactionsFromRecurring(month: string): Promise<number> {
  const db = await getDatabase();
  const items = await db.getAllAsync<Recurring>(
    `SELECT * FROM recurring WHERE active = 1 AND start_date <= ?`,
    [`${month}-31`],
  );
