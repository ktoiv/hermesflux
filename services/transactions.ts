...
import { CreateTransactionSchema, UpdateTransactionSchema, MonthString, type CreateTransaction, type UpdateTransaction } from '@/schemas';
...
export async function createTransaction(data: CreateTransaction): Promise<Transaction> {
  ...
  const result = await db.runAsync(
    `INSERT INTO transactions (name, amount, type, category_id, account_id, date, recurring_id, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [parsed.name, parsed.amount, parsed.type,
     parsed.category_id ?? null, parsed.account_id ?? null,
     parsed.date, parsed.recurring_id ?? null, parsed.notes ?? null],
  );
...
export async function updateTransaction(id: number, data: UpdateTransaction): Promise<Transaction | null> {
  ...
  await db.runAsync(
    ...
    [merged.name, merged.amount, merged.type, merged.category_id ?? null,
     merged.account_id ?? null, merged.date, merged.notes ?? null, id],
  );
...
export async function getCategoryBreakdown(month: string): Promise<Array<{
  category_id: number | null;
  category_name: string;
  total: number;
  percent: number;
