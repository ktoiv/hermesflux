...
export async function createLoan(data: CreateLoan): Promise<Loan> {
  ...
  const result = await db.runAsync(
    `INSERT INTO loans (name, lender, original_amount, remaining, interest_rate, monthly_payment, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [parsed.name, parsed.lender ?? null, parsed.original_amount ?? null,
     parsed.remaining, parsed.interest_rate, parsed.monthly_payment, parsed.notes ?? null],
  );
...
export async function updateLoan(id: number, data: UpdateLoan): Promise<Loan | null> {
  ...
  await db.runAsync(
    ...
    [merged.name, merged.lender ?? null, merged.original_amount ?? null,
     merged.remaining, merged.interest_rate, merged.monthly_payment, merged.portion, merged.notes ?? null, id],
  );
