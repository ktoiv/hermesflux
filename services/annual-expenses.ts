...
export async function createAnnualExpense(data: {
  ...
  const row = await db.getFirstAsync<AnnualExpense>('SELECT * FROM annual_expenses WHERE id = ?', [result.lastInsertRowId]);
  return row!;
}
...
export async function getMonthlyAnnualTotal(): Promise<number> {
  const db = await getDatabase();
  const row = await db.getFirstAsync<{ total: number }>('SELECT COALESCE(SUM(amount / 12.0), 0) as total FROM annual_expenses', []);
  return row?.total ?? 0;
}

export async function getAnnualBreakdown(): Promise<Array<{
  category_id: number | null;
