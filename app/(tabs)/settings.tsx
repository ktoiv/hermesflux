  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
          {THEME_OPTIONS.map((opt, i) => (
            <TouchableOpacity
              key={opt.value}
              style={[
                styles.optionRow,
                i < THEME_OPTIONS.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.surfaceContainerHighest },
              ]}
              onPress={() => setTheme(opt.value)}
              activeOpacity={0.7}
            >
              <View style={styles.optionLeft}>
                <MaterialIcons name={opt.icon as any} size={22} color={theme === opt.value ? colors.primary : colors.outline} />
        ...
        <Text style={[styles.sectionLabel, { color: colors.onSurfaceVariant, marginTop: Spacing.xl }]}>Currency</Text>

        <TouchableOpacity
          style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}
        ...
        >
          <View style={styles.optionRow}>
            ...
            <MaterialIcons name={showCurrencies ? 'expand-less' : 'expand-more'} size={22} color={colors.outline} />
          </View>
        </TouchableOpacity>

        {showCurrencies && (
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest, marginTop: Spacing.sm }]}>
            {CURRENCY_OPTIONS.map((opt, i) => (
              <TouchableOpacity
                key={opt.code}
                style={[
                  styles.optionRow,
                  i < CURRENCY_OPTIONS.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.surfaceContainerHighest },
                ]}
                onPress={() => { setCurrency(opt.code); setShowCurrencies(false); }}
                activeOpacity={0.7}
              >
                <View style={styles.optionLeft}>
                  <Text style={[styles.optionLabel, { color: colors.onSurface }]}>{opt.label}</Text>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest, marginTop: Spacing.xl }]}>
          <View style={styles.optionRow}>
            <View style={styles.optionLeft}>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing, textTransform: 'uppercase', marginBottom: Spacing.sm },
  card: { borderRadius: Radius.xl, borderWidth: 1, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.02, shadowRadius: 20, elevation: 1 },
  optionRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing.md, paddingVertical: 14 },
