...
function PositionRow({ position, index, colors, onEditPrice, onDelete, symbol = '$' }: { position: Position; index: number; colors: typeof Colors.light; onEditPrice: (p: Position) => void; onDelete: (id: number) => void; symbol?: string }) {
  ...
  return (
    <View style={[styles.positionCard, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
      <View style={styles.positionTop}>
        <View style={[styles.positionIcon, { backgroundColor: colors[colorKey] + '80' }]}>
          <Text style={[styles.positionSymbol, { color: colors.onSurface }]}>
            {position.symbol}
          </Text>
        </View>
        <View style={styles.positionInfo}>
          <Text style={[styles.positionName, { color: colors.onSurface }]}>{position.name ?? position.symbol}</Text>
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        <View style={[styles.summaryCard, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
          ...
          <Text style={[styles.summaryValue, { color: colors.onSurface }]}>{formatCurrency(totalValue, currencySymbol)}</Text>
          ...
          <View style={styles.summaryFooter}>
            <View>
              ...
              <Text style={[styles.summaryFooterValue, { color: colors.onSurface }]}>{formatCurrency(summary.totalInvested, currencySymbol)}</Text>
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={[styles.summaryFooterLabel, { color: colors.outline }]}>Positions</Text>
              <Text style={[styles.summaryFooterValue, { color: colors.onSurface }]}>{summary.positionCount}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.positionsList}>
          {positions.length > 0 ? (
            positions.map((position, i) => (
              <PositionRow key={position.id} position={position} index={i} colors={colors} onEditPrice={handleEditPrice} onDelete={handleDelete} symbol={currencySymbol} />
            ))
          ) : (
      ...
      <Modal visible={editing != null} transparent animationType="fade" onRequestClose={() => setEditing(null)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setEditing(null)}>
          <TouchableOpacity style={[styles.modalCard, { backgroundColor: colors.surface }]} activeOpacity={1} onPress={() => {}}>
            ...
            <TextInput
              style={[styles.modalInputField, { backgroundColor: colors.surfaceContainerLow, color: colors.onSurface }]}
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity onPress={handleSavePrice} style={[styles.modalBtn, { backgroundColor: colors.primary }]}>
...
const styles = StyleSheet.create({
  ...
  summaryLabel: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing, textTransform: 'uppercase', marginBottom: Spacing.xs },
  summaryValue: { fontSize: Typography.display.fontSize, fontWeight: Typography.display.fontWeight, lineHeight: Typography.display.lineHeight, letterSpacing: Typography.display.letterSpacing, marginBottom: Spacing.sm },
  ...
  summaryFooterLabel: { fontSize: Typography.bodySm.fontSize, fontWeight: Typography.bodySm.fontWeight, lineHeight: Typography.bodySm.lineHeight, marginBottom: 2 },
  ...
  positionCard: { borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.02, shadowRadius: 20, elevation: 1 },
  ...
  positionSymbol: { fontSize: Typography.labelMd.fontSize, fontWeight: '700', letterSpacing: Typography.labelMd.letterSpacing },
  positionInfo: { flex: 1 },
  positionName: { fontSize: Typography.bodyMd.fontSize, fontWeight: '500', lineHeight: Typography.bodyMd.lineHeight },
  positionDetail: { fontSize: Typography.bodySm.fontSize, fontWeight: Typography.bodySm.fontWeight, lineHeight: Typography.bodySm.lineHeight, marginTop: 1 },
  ...
  modalFieldLabel: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing, marginBottom: Spacing.sm },
  modalInputField: { fontSize: Typography.bodyMd.fontSize, lineHeight: Typography.bodyMd.lineHeight, paddingHorizontal: Spacing.md, paddingVertical: Platform.OS === 'ios' ? 14 : 10, borderRadius: Radius.lg, marginBottom: Spacing.lg },
  modalInputRow: { flexDirection: 'row', alignItems: 'center', borderRadius: Radius.lg, paddingHorizontal: Spacing.md, marginBottom: Spacing.lg },
  modalCurrency: { fontSize: Typography.h2.fontSize, fontWeight: '500', marginRight: Spacing.xs },
  modalInput: { flex: 1, fontSize: Typography.h2.fontSize, fontWeight: '500', paddingVertical: Platform.OS === 'ios' ? 14 : 10 },
