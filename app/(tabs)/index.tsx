...
function TransactionRow({ tx, colors, symbol = '$' }: { tx: TransactionWithJoins; colors: typeof Colors.light; symbol?: string }) {
  const isIncome = tx.type === 'income';
  const bgColor = tx.category_color
    ? (colors[tx.category_color as keyof typeof colors] as string) + '4D'
    : colors.surfaceContainerHigh;
  const iconColor = tx.category_color
    ? (colors[tx.category_color as keyof typeof colors] as string)
    : colors.outline;
  const amountColor = isIncome ? colors.primary : colors.onSurface;

  return (
    <View style={[styles.transactionRow, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
...
function DoughnutChart({ breakdown, colors, symbol = '$' }: { breakdown: Array<{ category_name: string; percent: number; total: number; color: string | null }>; colors: typeof Colors.light; symbol?: string }) {
  const cx = 18, cy = 18, r = 15.9;
    ...
    <View style={styles.chartContainer}>
      <View style={styles.chartSvgWrapper}>
        <Svg width={160} height={160} viewBox="0 0 36 36" style={{ transform: [{ rotate: '-90deg' }] }}>
          {segments.map((seg, i) => (
            <Circle
              ...
              r={r}
              fill="none"
              stroke={seg.color ? (colors[seg.color as keyof typeof colors] as string) : colors.surfaceContainerHigh}
              strokeWidth={4}
        ...
        <View style={styles.chartCenter}>
          ...
          <Text style={[styles.chartCenterAmount, { color: colors.onSurface }]}>{formatCurrency(totalSpent, symbol)}</Text>
        </View>
      </View>
        ...
        {breakdown.map((seg, i) => (
          <View key={i} style={styles.legendRow}>
            <View style={styles.legendLeft}>
              <View style={[styles.legendDot, { backgroundColor: seg.color ? (colors[seg.color as keyof typeof colors] as string) : colors.surfaceContainerHigh }]} />
              <Text style={[styles.legendLabel, { color: colors.onSurfaceVariant }]}>{seg.category_name}</Text>
            </View>
...
export default function DashboardScreen() {
  ...
  const [breakdown, setBreakdown] = useState<Array<{ category_name: string; percent: number; total: number; color: string | null }>>([]);
  ...
  useFocusEffect(
    useCallback(() => {
      (async () => {
        await Promise.all([
          generateTransactionsFromRecurring(ms),
          applyMonthlyPayments(ms),
        ]);
        ...
        const allSegments = [...catBreakdown, ...annualBreakdown.map(a => ({
        ...
        if (loanMonthlyTotal > 0) {
          allSegments.push({ category_id: null, category_name: 'Loan Payments', total: loanMonthlyTotal, percent: 0, color: 'tertiaryContainer' as const });
        }
        const grandTotal = allSegments.reduce((s, seg) => s + seg.total, 0);
        allSegments.forEach(seg => { seg.percent = grandTotal > 0 ? Math.round((seg.total / grandTotal) * 100) : 0; });
      ...
      >
        <View style={styles.monthSummary}>
          <View style={styles.monthSummaryLeft}>
            ...
            >
              {summary.net >= 0 ? '+' : ''}{formatCurrency(Math.abs(summary.net), currencySymbol)}
            </Text>
          </View>
        ...
        <View style={[styles.spendingSection, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
        ...
        <View style={styles.transactionsList}>
          {transactions.length > 0 ? (
            ...
            <Text style={[styles.emptyText, { color: colors.outline, textAlign: 'center', paddingVertical: Spacing.xl }]}>
              No transactions yet
            </Text>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  monthNav: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing.margin, paddingVertical: Spacing.sm, borderBottomWidth: 1 },
  monthArrow: { padding: Spacing.sm },
  monthLabel: { fontSize: Typography.h3.fontSize, fontWeight: Typography.h3.fontWeight, lineHeight: Typography.h3.lineHeight },
  scroll: { flex: 1 },
  ...
  monthSummaryLabel: { fontSize: Typography.bodySm.fontSize, fontWeight: Typography.bodySm.fontWeight, lineHeight: Typography.bodySm.lineHeight, marginBottom: 2 },
  monthSummaryAmount: { fontSize: Typography.h1.fontSize, fontWeight: Typography.h1.fontWeight, lineHeight: Typography.h1.lineHeight, letterSpacing: Typography.h1.letterSpacing },
  monthSummaryRight: { justifyContent: 'center', gap: Spacing.sm },
  monthSummaryRow: { alignItems: 'flex-end' },
  monthSummaryRowLabel: { fontSize: Typography.labelSm.fontSize, fontWeight: Typography.labelSm.fontWeight, lineHeight: Typography.labelSm.lineHeight, letterSpacing: Typography.labelSm.letterSpacing },
  ...
  sectionAction: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing },
  spendingSection: { borderRadius: Radius.xl, padding: Spacing.xl, marginBottom: Spacing.xl, borderWidth: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.04, shadowRadius: 20, elevation: 2 },
  cardTitle: { fontSize: Typography.h3.fontSize, fontWeight: Typography.h3.fontWeight, lineHeight: Typography.h3.lineHeight, marginBottom: Spacing.xl },
  emptyText: { fontSize: Typography.bodyMd.fontSize, fontWeight: Typography.bodyMd.fontWeight, lineHeight: Typography.bodyMd.lineHeight },
  chartContainer: { alignItems: 'center' },
  chartSvgWrapper: { width: 160, height: 160, marginBottom: Spacing.xl, position: 'relative' },
  chartCenter: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center' },
  chartCenterLabel: { fontSize: Typography.labelSm.fontSize, fontWeight: Typography.labelSm.fontWeight, lineHeight: Typography.labelSm.lineHeight, letterSpacing: Typography.labelSm.letterSpacing },
  chartCenterAmount: { fontSize: Typography.h3.fontSize, fontWeight: Typography.h3.fontWeight, lineHeight: Typography.h3.lineHeight },
  ...
  legendLabel: { fontSize: Typography.bodySm.fontSize, fontWeight: Typography.bodySm.fontWeight, lineHeight: Typography.bodySm.lineHeight },
  legendPercent: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing },
  quickActions: { flexDirection: 'row', gap: Spacing.sm, marginBottom: Spacing.md },
  quickAction: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm + 2, borderRadius: Radius.full },
  ...
  transactionRow: { borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.02, shadowRadius: 20, elevation: 1 },
  ...
  transactionName: { fontSize: Typography.bodyMd.fontSize, fontWeight: '500', lineHeight: Typography.bodyMd.lineHeight },
  transactionDate: { fontSize: Typography.bodySm.fontSize, fontWeight: Typography.bodySm.fontWeight, lineHeight: Typography.bodySm.lineHeight },
  transactionAmount: { fontSize: Typography.bodyMd.fontSize, fontWeight: '600', lineHeight: Typography.bodyMd.lineHeight },
});
