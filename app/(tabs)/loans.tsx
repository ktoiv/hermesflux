...
function LoanCard({ loan, colors, colorKey, symbol = '$', onEdit, onDelete }: { loan: Loan; colors: typeof Colors.light; colorKey: keyof typeof Colors.light; symbol?: string; onEdit: (l: Loan) => void; onDelete: (id: number) => void }) {
  const paidPercent = loan.original_amount && loan.original_amount > 0
  ...
  return (
    <View style={[styles.loanCard, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
      ...
      <View style={styles.loanAmounts}>
        <View>
          ...
          <Text style={[styles.loanAmountValue, { color: colors.onSurface }]}>{formatCurrency(loan.remaining, symbol)}</Text>
        </View>
      ...
      {loan.original_amount && loan.original_amount > 0 && (
        <>
          <View style={[styles.progressTrack, { backgroundColor: colors.surfaceContainerLow }]}>
            <View style={[styles.progressBar, { backgroundColor: colors[colorKey], width: `${paidPercent}%` }]} />
          </View>
          <View style={styles.progressLabels}>
            <Text style={[styles.progressText, { color: colors.outline }]}>{paidPercent.toFixed(0)}% paid</Text>
            <Text style={[styles.progressText, { color: colors.outline }]}>{formatCurrency(loan.original_amount, symbol)}</Text>
          </View>
        </>
      )}
      ...
      <View style={styles.loanFooter}>
        <View style={styles.loanFooterItem}>
          ...
          <Text style={[styles.loanFooterValue, { color: colors.onSurface }]}>{formatCurrency(loan.monthly_payment, symbol)}</Text>
        </View>
        {loan.original_amount && (
          <View style={styles.loanFooterItem}>
            <Text style={[styles.loanFooterLabel, { color: colors.outline }]}>Total</Text>
            <Text style={[styles.loanFooterValue, { color: colors.onSurface }]}>{formatCurrency(loan.original_amount, symbol)}</Text>
          </View>
        )}
      </View>
    </View>
...
export default function LoansScreen() {
  ...
  const [summary, setSummary] = useState({ totalRemaining: 0, totalMonthlyPayment: 0, loanCount: 0, avgInterestRate: 0 });
  ...
  const handleSaveEdit = async () => {
    ...
    if (isNaN(rate) || rate < 0 || isNaN(payment) || payment <= 0 || isNaN(portion) || portion < 0 || portion > 100) return;
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        <View style={[styles.summaryCard, { backgroundColor: colors.surface, borderColor: colors.surfaceContainerHighest }]}>
          <View style={styles.summaryRow}>
            <View style={styles.summaryItem}>
              ...
              <Text style={[styles.summaryValue, { color: colors.onSurface }]}>{formatCurrency(summary.totalRemaining, currencySymbol)}</Text>
            </View>
            <View style={styles.summaryItem}>
              <Text style={[styles.summaryLabel, { color: colors.outline }]}>Monthly Payments</Text>
              <Text style={[styles.summaryValue, { color: colors.primary }]}>{formatCurrency(summary.totalMonthlyPayment, currencySymbol)}</Text>
            </View>
          </View>
          ...
          <View style={styles.summaryRow}>
            ...
            <View style={styles.summaryItem}>
              ...
              <Text style={[styles.summaryValue, { color: colors.onSurface }]}>{summary.avgInterestRate}%</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.loansList}>
          {loans.length > 0 ? (
            loans.map((loan, i) => (
              <LoanCard key={loan.id} loan={loan} colors={colors} colorKey={COLORS[i % COLORS.length]} symbol={currencySymbol} onEdit={handleEdit} onDelete={handleDelete} />
            ))
          ) : (
      ...
      <Modal visible={editing != null} transparent animationType="fade" onRequestClose={() => setEditing(null)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setEditing(null)}>
          <TouchableOpacity style={[styles.modalCard, { backgroundColor: colors.surface }]} activeOpacity={1} onPress={() => {}}>
            <Text style={[styles.modalTitle, { color: colors.onSurface }]}>{editing?.name}</Text>

            <Text style={[styles.modalFieldLabel, { color: colors.onSurfaceVariant }]}>Interest Rate (%)</Text>
            <TextInput
            ...
            <Text style={[styles.modalFieldLabel, { color: colors.onSurfaceVariant }]}>Monthly Payment ({currencySymbol})</Text>
            <TextInput
              style={[styles.modalInput, { backgroundColor: colors.surfaceContainerLow, color: colors.onSurface }]}
              keyboardType="decimal-pad"
              value={editPayment}
            ...
            <Text style={[styles.modalFieldLabel, { color: colors.onSurfaceVariant }]}>Your Portion (%)</Text>
            <TextInput
              style={[styles.modalInput, { backgroundColor: colors.surfaceContainerLow, color: colors.onSurface }]}
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setEditing(null)} style={[styles.modalBtn, { backgroundColor: colors.surfaceContainerHigh }]}>
...
const styles = StyleSheet.create({
  ...
  loanCard: { borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.02, shadowRadius: 20, elevation: 1 },
  ...
  loanAmountLabel: { fontSize: Typography.labelSm.fontSize, fontWeight: Typography.labelSm.fontWeight, lineHeight: Typography.labelSm.lineHeight, letterSpacing: Typography.labelSm.letterSpacing, marginBottom: 2 },
  ...
  loanFooterLabel: { fontSize: Typography.labelSm.fontSize, fontWeight: Typography.labelSm.fontWeight, lineHeight: Typography.labelSm.lineHeight, letterSpacing: Typography.labelSm.letterSpacing, marginBottom: 2 },
  ...
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center', padding: Spacing.xl },
  ...
  modalFieldLabel: { fontSize: Typography.labelMd.fontSize, fontWeight: Typography.labelMd.fontWeight, lineHeight: Typography.labelMd.lineHeight, letterSpacing: Typography.labelMd.letterSpacing, marginBottom: Spacing.sm, marginTop: Spacing.md },
  modalInput: { fontSize: Typography.bodyMd.fontSize, lineHeight: Typography.bodyMd.lineHeight, paddingHorizontal: Spacing.md, paddingVertical: Platform.OS === 'ios' ? 14 : 10, borderRadius: Radius.lg },
