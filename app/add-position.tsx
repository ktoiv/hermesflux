...
export default function AddPositionScreen() {
  ...
  const handleSave = async () => {
    ...
    const pos = await createPosition({ symbol: symbol.trim(), name: name.trim() || null, shares: parseFloat(shares), avg_cost: parseFloat(avgCost), notes: notes.trim() || null });
    await updateCurrentPrice(pos.id, parseFloat(avgCost));
    router.back();
  };

  const canSave = symbol.trim().length > 0 && shares.trim().length > 0 && parseFloat(shares) > 0 && avgCost.trim().length > 0 && parseFloat(avgCost) > 0;
      ...
      >
        ...
        >
          ...
          <View style={styles.splitRow}>
            ...
            <View style={styles.splitField}>
              <Text style={[styles.fieldLabel, { color: colors.onSurfaceVariant }]}>Avg. Cost / Share</Text>
          ...
          {shares && avgCost && parseFloat(shares) > 0 && parseFloat(avgCost) > 0 && (
            <View style={[styles.totalPreview, { backgroundColor: colors.surfaceContainerLow }]}>
              ...
              <Text style={[styles.totalPreviewValue, { color: colors.onSurface }]}>
                {currencySymbol}{(parseFloat(shares) * parseFloat(avgCost)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </Text>
            </View>
          )}
